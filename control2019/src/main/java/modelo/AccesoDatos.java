...
public class AccesoDatos {
    ...
    private final ArrayList <Movimiento> listamov;
    ...
    // Creo una lista de Movimientos, podría obtenerse de una base de datos
    private AccesoDatos (){
    ...
    /*
     * DEVUELVE UN NUEVO ARRAYLIST CON LOS MOVIMIENTOS QUE CUMPLEN LA CONDICIÓN
     * DEVUELVE UN ARRAYLIST VACÍO SI NO HAY MOVIMIENTOS PARA ESE CLIENTE  resultado.size() == 0
     */
    ...
    public ArrayList <Movimiento> obtenerListaMovimientos (String cod_cliente, int importmax, int importmin){
        ...
        for (Movimiento m : listamov) {
            if (m.getCod_cliente().equals(cod_cliente) && m.getImporte() <= importmax && m.getImporte() >= importmin) {
                resultado.add(m);
            }
        }
    ...
    // Devuelve TRUE o FALSE su hay algun Movimiento entre los importes dados
    public boolean hayMovimientoImporte (String cod_cliente, int max, int min){
        boolean resu = false;

        for (Movimiento m : listamov) {
            if (m.getCod_cliente().equals(cod_cliente) && m.getImporte() <= max && m.getImporte() >= min) {
                resu = true;
                break;
            }
        }
